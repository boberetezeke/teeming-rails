...
describe MemberGroup do
  ...
  let!(:user_state)     { FactoryBot.create(:user,   email: 'user@c.com', chapter: chapter_state) }
  ...
  describe "#all_members" do
    context "when selecting chapter members in the state chapter" do
      it "selects all members" do
        member_group = MemberGroup.new(scope_type: :all_members, group_type: MemberGroup::GROUP_TYPE_SCOPE)
        expect(member_group.all_members(chapter_state)).to match_array([
          member, member_state, member_state_p, member_other,
          user.member, user_other.member, user_state.member,
          user_officer.member, user_officer_out_of_date.member, user_board_member.member,
          user_exec_cmmt_member.member, user_exec_cmmt_member_other.member])
      end

      it "selects all users" do
        member_group = MemberGroup.new(scope_type: :all_users, group_type: MemberGroup::GROUP_TYPE_SCOPE)
        expect(member_group.all_members(chapter_state)).to match_array([
           user.member, user_other.member, user_state.member,
           user_officer.member, user_officer_out_of_date.member, user_board_member.member,
           user_exec_cmmt_member.member, user_exec_cmmt_member_other.member])
      ...
      it "selects state chapter members" do
        member_group = MemberGroup.new(scope_type: :chapter_members, group_type: MemberGroup::GROUP_TYPE_SCOPE)
        expect(member_group.all_members(chapter_state)).to match_array([member_state, member_state_p, user_state.member])
      end

      it "selects state chapter users" do
        member_group = MemberGroup.new(scope_type: :chapter_users, group_type: MemberGroup::GROUP_TYPE_SCOPE)
        expect(member_group.all_members(chapter_state)).to match_array([user_state.member])
      end
    end

    context "when selecting chapter users" do
      let(:member_group) { MemberGroup.new(scope_type: :chapter_users, group_type: MemberGroup::GROUP_TYPE_SCOPE) }
      it "selects chapter members" do
        expect(member_group.all_members(chapter)).to match_array([user.member, user_officer.member, user_officer_out_of_date.member, user_board_member.member, user_exec_cmmt_member.member])
      end

      it "selects other chapter members" do
        mg = MemberGroup.new(scope_type: :chapter_members, group_type: MemberGroup::GROUP_TYPE_SCOPE)
        expect(member_group.all_members(other_chapter)).to match_array([user_other.member, user_exec_cmmt_member_other.member])
      end
    end
