...
describe VotesController do
  ...
  let!(:questionnaire_section_1) { FactoryBot.create(:questionnaire_section, questionnaire: questionnaire, order_index: 1, title: 'Section 1') }
  ...
  let!(:ranked_choice_1)         { FactoryBot.create(:choice, question: ranked_choice_question, title: 'choice 1', order_index: 1) }
  let!(:ranked_choice_2)         { FactoryBot.create(:choice, question: ranked_choice_question, title: 'choice 2', order_index: 2) }
  ...
  let(:election)                 { FactoryBot.create(:election, :internal, :online_only,
  ...
  let(:oao_election)             { FactoryBot.create(:election, :internal, :online_and_offline,
                                                      questionnaire: questionnaire,
                                                      vote_date: Date.new(2017, 3, 10),
                                                      vote_start_time: Time.zone.local(2017, 3, 10, 16, 00),
                                                      vote_end_time: Time.zone.local(2017, 3, 10, 16, 30))
  }
  let(:offline_election)         { FactoryBot.create(:election, :internal, :offline_only, questionnaire: questionnaire)}
  let(:race)                     { FactoryBot.create(:race, election: election) }
  let(:user)                     { FactoryBot.create(:user) }
  let(:offline_enter_role)       { FactoryBot.create(:role, privileges: [FactoryBot.create(:privilege, subject: 'vote', action: 'enter')])}
  let(:offline_enter_user)       { FactoryBot.create(:user, email: 'offline@enter.org', role: offline_enter_role)}
  ...
  describe "vote" do
    ...
    it "does not redirect if it is between vote start and end near the beginning" do
      FactoryBot.create(:vote_completion, election: election, user: user)
  ...
  describe "download_votes" do
    let!(:ranked_choice_question_2)  { FactoryBot.create(:question, questionnaire_section: questionnaire_section_1, question_type: Question::QUESTION_TYPE_RANKED_CHOICE, order_index: 2) }
    ...
    let!(:vote_completion_3)         { FactoryBot.create(:vote_completion, election: election, user: vote_user_3, has_voted: nil, vote_type: VoteCompletion::VOTE_COMPLETION_TYPE_ONLINE )}

    let!(:answer_1_1)                 { FactoryBot.create(:answer, question: ranked_choice_question,   text: "0:::1", user: vote_user_1, answerable: vote_completion_1) }
    let!(:answer_1_2)                 { FactoryBot.create(:answer, question: ranked_choice_question_2, text: "1:::0", user: vote_user_1, answerable: vote_completion_1) }

    let!(:answer_2_1)                 { FactoryBot.create(:answer, question: ranked_choice_question,   text: "1:::0", user: vote_user_2, answerable: vote_completion_2) }
    let!(:answer_2_2)                 { FactoryBot.create(:answer, question: ranked_choice_question_2, text: "1:::0", user: vote_user_2, answerable: vote_completion_2) }
